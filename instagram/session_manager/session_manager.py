...
class SessionManager:
    ...
    def _load_device_params(self):
        ...
        try:
            # Пытаемся загрузить из файла, если он существует
            if os.path.exists('data/device_params.json'):
                with open('data/device_params.json', 'r') as f:
                    return json.load(f)
            # Иначе возвращаем базовый список
            return [
                {
    ...
    def check_session_health(self, client, username):
        """
        Проверка здоровья сессии через запросы к Instagram API

        Args:
            client: Объект клиента instagrapi
        ...
        try:
            # Пробуем выполнить простой запрос к API Instagram
            # Используем методы клиента instagrapi вместо session.get
            try:
                # Пробуем получить ленту
                ...
                if "challenge" in error_message:
                ...
                elif "login" in error_message or "authenticate" in error_message:
                    ...
                    self.logger.error(f"Unknown error for {username}: {error_message}")
